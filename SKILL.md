...
- 生成优化创新点、模块替换、涨点方案或新模型框架时，同时读取根目录下的 `整理的模块.md`，把它作为通用可迁移模块库参考。使用时优先提取每个候选模块的作用、机制、独特优势和代码接入方式，再判断是否适合当前论文任务。
...
使用 `整理的模块.md` 时，不要把模块清单机械堆叠成创新点；必须说明候选模块与论文失败模式、特征瓶颈、计算成本或指标目标之间的对应关系。若模块只是常见注意力/卷积/融合组件，要进一步给出任务化改造、插入位置、接口对齐和消融验证方式。
...
- **模块库参考**：可从 `整理的模块.md` 中筛选 2-4 个候选模块作为备选灵感，但最终推荐只保留与当前 baseline 和失败模式强相关的模块，并解释为什么选它、不选其他模块。
