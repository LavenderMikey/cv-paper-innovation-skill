...
然后代码最好也要是每行都要注释清楚。
是吧，都是这样过来的。我之前也是这样想的。但是没有，所以我就把这个工作，做起来。我整合了 差不多有70个通用模块，从经典的模块，到现在近几年的最新的模块。每个模块，大致讲解一下，模块的内容。还是很重要，因为我们把模块拿过来了，自己也要去写论文，那么写论文就要遇见这些问题，所以我提前写好。大家能大致有个思路，仿写。然后每个模块的代码我基本注释了说明！
...
EGA 模块旨在针对低质量遥感图像中目标边界模糊、噪声干扰强、特征响应不稳定的问题，通过引入边缘与高斯先验，引导网络获得更加清晰、稳定且具有判别性的特征表示，从而提升目标检测在复杂成像条件下的鲁棒性。



### 2、机制

EGA 模块通过分层引入固定先验信息来增强特征表达能力：在浅层特征中利用 Scharr 算子显式强化边缘信息，以补偿因分辨率下降或模糊造成的结构缺失；在深层特征中引入高斯建模，对特征响应进行平滑与正则化，以抑制噪声并稳定语义分布。模块通过轻量卷积与残差连接将先验信息与原始特征融合，在不显著增加学习难度的情况下提升特征质量。
...
1、**显式先验引导**：
 EGA 模块直接引入经典边缘算子与高斯先验，而非完全依赖数据驱动学习，在低质量图像条件下具有更强的稳定性和可解释性。

2、**分层设计合理**：
 边缘增强仅作用于浅层，高斯建模用于深层特征，避免边缘信息在高层语义中特征干扰，符合卷积网络的层级表达规律。

3、**轻量高效**：
 模块采用固定算子与轻量卷积结构，不显著增加参数量与计算量，适合资源受限的遥感与边缘部署场景。

4、**通用性强**：
 EGA 模块可作为独立特征增强单元嵌入现有检测骨干网络中，无需对整体架构进行大规模调整。
...
from mmcv.cnn import build_norm_layer  # MMCV 中的归一化层构建函数（BN / GN / LN 等）
import math                          # 数学函数（用于高斯公式）


class Conv_Extra(nn.Module):
    def __init__(self, channel, norm_layer, act_layer):
        super(Conv_Extra, self).__init__()   # 初始化 nn.Module 父类
        self.block = nn.Sequential(
            nn.Conv2d(channel, 64, 1),                       # 1×1 卷积：通道压缩 / 映射
            build_norm_layer(norm_layer, 64)[1],             # 归一化层（根据配置动态构建）
            act_layer(),                                     # 激活函数（如 ReLU）
            nn.Conv2d(64, 64, 3, stride=1, padding=1, dilation=1, bias=False),  # 3×3 卷积：局部空间建模
            build_norm_layer(norm_layer, 64)[1],             # 再次归一化
            act_layer(),                                     # 再次激活
            nn.Conv2d(64, channel, 1),                       # 1×1 卷积：恢复原始通道数
            build_norm_layer(norm_layer, channel)[1]         # 输出通道归一化
        )

    def forward(self, x):
        out = self.block(x)      # 输入特征经过额外卷积增强模块
        return out               # 返回增强后的特征
...
class EdgeGaussianAggregation(nn.Module):
    def __init__(self, dim, size, sigma, norm_layer, act_layer, feature_extra=True):
        super().__init__()
        self.feature_extra = feature_extra    # 是否启用额外特征增强分支

        gaussian = self.gaussian_kernel(size, sigma)          # 生成二维高斯核
        gaussian = nn.Parameter(data=gaussian, requires_grad=False).clone()  
        # 将高斯核注册为参数，但不参与训练（固定滤波器）
        ...
        self.gaussian = nn.Conv2d(
            ...
            stride=1,
            padding=int(size // 2),
            groups=dim,           # 深度可分离卷积：每个通道独立做高斯滤波
            bias=False
        )
        self.gaussian.weight.data = gaussian.repeat(dim, 1, 1, 1)
        # 将同一个高斯核复制到每个通道，实现通道独立的高斯平滑

        self.norm = build_norm_layer(norm_layer, dim)[1]  # 高斯响应后的归一化
        self.act = act_layer()                             # 激活函数

        if feature_extra == True:
            self.conv_extra = Conv_Extra(dim, norm_layer, act_layer)
            # 可选的特征增强模块，用于融合原特征与高斯边缘特征

    def forward(self, x):
        edges_o = self.gaussian(x)        # 对输入特征做高斯卷积（边缘 / 结构响应）
        gaussian = self.act(self.norm(edges_o))  # 归一化 + 激活后的高斯特征

        if self.feature_extra == True:
            out = self.conv_extra(x + gaussian)  
            # 原始特征与高斯特征残差融合后，再做额外卷积增强
        else:
            out = gaussian                # 仅输出高斯特征
    ...
    def gaussian_kernel(self, size: int, sigma: float):
        kernel = torch.FloatTensor([
            [
                (1 / (2 * math.pi * sigma ** 2)) *
                math.exp(-(x ** 2 + y ** 2) / (2 * sigma ** 2))   # 二维高斯分布公式
                for x in range(-size // 2 + 1, size // 2 + 1)
            ]
            for y in range(-size // 2 + 1, size // 2 + 1)
        ]).unsqueeze(0).unsqueeze(0)
        # shape: [1, 1, size, size]，符合 Conv2d 权重格式

        return kernel / kernel.sum()   # 归一化，保证高斯核权重和为 1
...
if __name__ == "__main__":
    ...
    channels = 3                       # 输入通道数
    height, width = 256, 256           # 输入特征图尺寸
    size = 3                           # 高斯核大小
    sigma = 1.0                        # 高斯分布标准差
    norm_layer = dict(type='BN', requires_grad=True)  # 使用 BatchNorm
    act_layer = nn.ReLU                # 使用 ReLU 激活

    # 创建随机输入张量
    input_tensor = torch.randn(batch_size, channels, height, width)

    # 初始化 EdgeGaussianAggregation 模块
    gaussian = EdgeGaussianAggregation(
        dim=channels,
        size=size,
        sigma=sigma,
...
论文《SalM²: An Extremely Lightweight Saliency Mamba Model for Real-Time Cognitive Awareness of Driver Attention》

### 1、作用

SalM2 旨在以极低参数量和计算成本实现对驾驶员注意力分布的精准预测，通过引入驾驶场景语义信息来引导模型关注与当前驾驶任务最相关的区域，从而在复杂、动态交通场景中实现实时、可靠的注意力感知。



### 2、机制

SalM2 采用“Bottom-up + Top-down”双分支机制： Bottom-up 分支基于 Mamba 构建超轻量骨干网络，用于提取驾驶场景的视觉特征；Top-down 分支利用预训练的 CLIP 模型提取与驾驶任务相关的语义信息。在图像特征提取的最深层，通过跨模态注意力机制（CMA）将语义信息映射并融合到视觉特征中，从而引导注意力分配。 同时，引入选择性通道并行 Mamba（SCPM）结构，在避免高维状态空间参数爆炸的前提下，增强通道级建模能力。
...
1、**极致轻量化**：
 SalM2 仅包含约 0.08M 可训练参数，在保持 SOTA 或接近 SOTA 性能的同时，显著优于现有注意力预测模型的参数效率。

2、**语义驱动注意力建模**：
 通过显式引入驾驶任务相关的语义信息，使注意力预测不再依赖纯视觉显著性，更贴近真实驾驶员的认知行为。

3、**Mamba 架构高效利用**：
 基于 Mamba 的线性时间建模能力，结合 SCPM 设计，有效提升通道表达能力，同时避免传统 Mamba 在高维视觉任务中的参数冗余问题。

4、**强实时与可部署性**：
 低 FLOPs、低显存占用的设计，使模型适合部署在实际驾驶辅助系统和资源受限的实时场景中。
...
import torch.nn.functional as F          # 常用函数接口（此处未直接使用）

# ================== 交叉模态注意力模块 ==================
class CrossModelAtt(nn.Module):
    def __init__(self, feature_dim, height, width):
        super().__init__()
        self.gamma = nn.Parameter(torch.zeros(1))   # 可学习的缩放系数，控制注意力分支的影响强度
        self.softmax = nn.Softmax(dim=-1)            # 在最后一个维度上做 Softmax，生成注意力权重

    def forward(self, img_feat, text_feat):
        # img_feat: 图像特征图 [B, C, H, W]
        # text_feat: 文本特征图 [B, C, H, W]
        B, C, H, W = img_feat.shape                  # 读取批量大小、通道数和空间尺寸

        # -------- Step 1: 特征展平 --------
        q = img_feat.view(B, C, -1)                  # Query，图像特征展平为 [B, C, H*W]
        k = text_feat.view(B, C, -1).permute(0, 2, 1) # Key，文本特征展平并转置为 [B, H*W, C]

        # -------- Step 2: 计算跨模态注意力矩阵 --------
        attention_map = torch.bmm(q, k)              # 批量矩阵乘法，得到 [B, C, C] 的通道级相关性
        attention_map = self.softmax(attention_map)  # Softmax 归一化，生成注意力权重

        # -------- Step 3: 特征融合 --------
        v = text_feat.view(B, C, -1)                 # Value，文本特征展平为 [B, C, H*W]
        attention_info = torch.bmm(attention_map, v) # 加权求和，得到融合特征 [B, C, H*W]

        # -------- Step 4: 恢复空间结构 --------
        attention_info = attention_info.view(B, C, H, W)  # 重构回 [B, C, H, W]

        # -------- Step 5: 残差融合 --------
        output = self.gamma * attention_info + img_feat   # 用可学习参数调节注意力强度并与原特征相加

        return output                                     # 输出融合后的图像特征
...
if __name__ == "__main__":
    ...
    # 创建图像和文本特征图张量
    img_feat = torch.randn(batch_size, channels, height, width).cuda()   # 模拟图像特征输入
    text_feat = torch.randn(batch_size, channels, height, width).cuda()  # 模拟文本特征输入

    # 初始化交叉模态注意力模块
    cross_attention = CrossModelAtt(
        feature_dim=channels,
        height=height,
        width=width
    ).cuda()
    print(cross_attention)                            # 打印网络结构

    # 前向传播测试
    output = cross_attention(img_feat, text_feat)     # 执行交叉模态注意力计算

    # 打印输入和输出的形状
    print(f"Input image feature shape: {img_feat.shape}")  # 图像特征尺寸
    print(f"Input text feature shape : {text_feat.shape}") # 文本特征尺寸
    print(f"Output shape : {output.shape}")                 # 融合后输出尺寸
...
论文《Regional Attention for Shadow Removal》

### 1、作用

Regional Attention 模块旨在高效利用阴影区域周围的非阴影信息，引导模型重点关注对阴影修复最关键的局部上下文，从而在保证高质量阴影去除效果的同时，显著降低模型的计算复杂度与参数规模。



### 2、机制

Regional Attention 通过以每个像素为中心的区域级注意力建模，仅在其邻域范围内进行信息交互。与全局自注意力或固定窗口注意力不同，该机制为每个阴影位置动态选择与之最相关的周边非阴影区域进行特征聚合，实现“按位置定制的局部注意力”。同时，通过可选的空洞（dilation）策略，在不显著增加计算量的前提下扩大感受野，增强阴影与非阴影区域之间的上下文关联建模能力。
...
1、**贴合任务本质**：
 针对“阴影属于局部退化而非全局退化”的特点设计，避免无关区域参与注意力计算，更符合阴影去除的实际需求。

2、**效率与效果平衡**：
 相比全局自注意力，大幅降低计算复杂度；相比窗口注意力，具备更灵活的空间适应性，不易遗漏关键非阴影线索。

3、**位置自适应建模**：
 每个阴影位置均可聚焦不同的周边非阴影区域，避免固定窗口带来的信息不足或无效聚合问题。

4、**易于集成**：
 作为独立注意力模块，可直接嵌入 U-Net 或 Transformer 风格的图像恢复网络中，无需复杂结构改动。
...
import torch                         # PyTorch 主库
import torch.nn as nn                # 神经网络模块
import math                          # 数学库（用于通道数计算）

# ================== Squeeze-and-Excitation 模块 ==================
class SELayer(nn.Module):
    def __init__(self, channel, reduction=4):
        super(SELayer, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)    # 全局平均池化，将空间维度压缩为 1×1
        self.fc = nn.Sequential(
            nn.Linear(channel, channel // reduction),  # 通道压缩，降低参数量
            nn.ReLU(inplace=True),                      # 非线性激活
            nn.Linear(channel // reduction, channel), )# 通道恢复，生成通道权重

    def forward(self, x):
        b, c, _, _ = x.size()                        # 获取 batch 和通道数
        y = self.avg_pool(x).view(b, c)              # 全局池化并展平为 [B, C]
        y = self.fc(y).view(b, c, 1, 1)              # 全连接映射并恢复为通道权重
        y = torch.clamp(y, 0, 1)                     # 将权重限制在 [0,1] 区间
        return x * y                                 # 对输入特征进行通道级重标定
...
def depthwise_conv(inp, oup, kernel_size=3, stride=1, relu=False):
    return nn.Sequential(
        nn.Conv2d(
            inp, oup, kernel_size, stride,
            kernel_size // 2,
            groups=inp,                               # 深度卷积：每个输入通道独立卷积
            bias=False
        ),
        nn.BatchNorm2d(oup),                          # 批归一化
        nn.ReLU(inplace=True) if relu else nn.Sequential(),  # 可选激活函数
    )
...
class GhostModule(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, relu=True):
        ...
        self.oup = oup                                # 输出通道数
        init_channels = math.ceil(oup / ratio)       # 主分支通道数
        new_channels = init_channels * (ratio - 1)   # Ghost 特征通道数
        ...
        self.cheap_operation = nn.Sequential(
            nn.Conv2d(
                init_channels, new_channels, dw_size, 1,
                dw_size // 2,
                groups=init_channels,                # 深度卷积生成 Ghost 特征
                bias=False
            ),
    ...
    def forward(self, x):
        x1 = self.primary_conv(x)                     # 基础特征
        x2 = self.cheap_operation(x1)                 # Ghost 特征
        out = torch.cat([x1, x2], dim=1)              # 拼接特征
        return out[:, :self.oup, :, :]                # 裁剪至目标通道数
...
class GhostBottleneck(nn.Module):
    def __init__(self, inp, hidden_dim, oup, kernel_size, stride):
        ...
        assert stride in [1, 2]                       # 限制步长取值

        self.conv = nn.Sequential(
            # pw：Ghost Module 扩展通道
            GhostModule(inp, hidden_dim, kernel_size=1, relu=True),

            # dw：深度卷积（stride=2 时进行下采样）
            ...
            # pw-linear：Ghost Module 压缩通道
            GhostModule(hidden_dim, oup, kernel_size=1, relu=False),
        )

        if stride == 1 and inp == oup:
            self.shortcut = nn.Sequential()           # 维度一致，直接残差连接
        else:
            self.shortcut = nn.Sequential(
                depthwise_conv(inp, inp, kernel_size, stride, relu=False),  # 下采样或对齐空间尺寸
                nn.Conv2d(inp, oup, 1, 1, 0, bias=False),                     # 通道对齐
                nn.BatchNorm2d(oup),
            )

    def forward(self, x):
        return self.conv(x) + self.shortcut(x)        # 主分支与捷径分支相加
...
if __name__ == "__main__":
    ...
    width = 256
    kernel_size = 3
    stride = 1                                       # 可改为 2 测试下采样

    # 构造输入张量
    x = torch.randn(batch_size, inp_channels, height, width).cuda()

    # 实例化 GhostBottleneck 模块
...
SCSA 模块旨在通过多语义引导的通道—空间联合建模，缓解不同语义特征之间的干扰问题，使网络能够同时关注多个关键区域与语义模式，从而提升模型在分类、分割与检测等视觉任务中的特征表达一致性与判别能力。



### 2、机制

SCSA 由 SMSA（Spatial Multi-Semantic Attention） 与 PCSA（Progressive Channel Semantic Attention） 两个子模块串联构成。 SMSA 通过多分支、多尺度卷积在空间维度上显式建模不同语义子特征，引导网络关注多语义区域；随后，PCSA 在通道维度上对这些语义特征进行交互与重标定，以缓解多语义空间建模带来的通道语义差异。整体采用“先空间语义引导，再通道语义融合”的设计顺序，实现空间与通道信息的协同增强。
...
1、**多语义建模能力强**：
 通过多分支、多尺度结构显式捕获不同语义子特征，避免单一注意力机制对复杂语义模式建模不足的问题。

2、**空间引导通道的协同设计**：
 采用先空间、后通道的注意力顺序，使空间语义信息有效指导通道特征学习，减少语义冲突与信息干扰。

3、**兼顾性能与效率**：
 在参数与 FLOPs 基本不增加的前提下，相比 SE、ECA、CBAM 等注意力模块取得更稳定的性能提升。

4、**良好的通用性**：
 SCSA 作为即插即用模块，可直接嵌入 ResNet、UperNet、Mask R-CNN 等主流架构中，适用于多种视觉任务。
...
class Shareable_Multi_Semantic_Spatial_Attention(nn.Module):
    def __init__(
            self,
            dim: int,                                   # 输入特征通道数
            group_kernel_sizes: t.List[int] = [3, 5, 7, 9],  # 不同语义分支的卷积核尺寸
            gate_layer: str = 'sigmoid',                # 注意力门控函数类型
    ):
        super(Shareable_Multi_Semantic_Spatial_Attention, self).__init__()
        self.dim = dim                                  # 保存输入通道数

        assert self.dim % 4 == 0, '输入特征的维度应能被4整除。'  # 确保可均分为4个语义组
        self.group_chans = group_chans = self.dim // 4  # 每个语义组的通道数
        ...
        self.local_dwc = nn.Conv1d(
            ...
            groups=group_chans                           # 深度可分离卷积（局部感受野）
        )
        ...
        self.global_dwc_l = nn.Conv1d(
            ...
            groups=group_chans                           # 大尺度全局语义建模
        )

        self.sa_gate = nn.Softmax(dim=2) if gate_layer == 'softmax' else nn.Sigmoid()
        # 选择注意力权重生成方式：Softmax（竞争关系）或 Sigmoid（独立门控）

        self.norm_h = nn.GroupNorm(4, dim)               # 高度方向的组归一化
        self.norm_w = nn.GroupNorm(4, dim)               # 宽度方向的组归一化

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h_, w_ = x.size()                          # 读取输入尺寸 [B, C, H, W]

        # ================== 高度方向语义建模 ==================
        x_h = x.mean(dim=3)                              # 沿宽度方向池化，得到 [B, C, H]
        l_x_h, g_x_h_s, g_x_h_m, g_x_h_l = torch.split(
            x_h, self.group_chans, dim=1                 # 按通道划分为 4 个语义子空间
        )

        # ================== 宽度方向语义建模 ==================
        x_w = x.mean(dim=2)                              # 沿高度方向池化，得到 [B, C, W]
        l_x_w, g_x_w_s, g_x_w_m, g_x_w_l = torch.split(
            x_w, self.group_chans, dim=1                 # 同样划分为 4 个语义子空间
        )
        ...
        x_h_attn = self.sa_gate(
            self.norm_h(
                torch.cat((
                    self.local_dwc(l_x_h),               # 局部语义分支
                    self.global_dwc_s(g_x_h_s),          # 小尺度全局语义
                    self.global_dwc_m(g_x_h_m),          # 中尺度全局语义
                    self.global_dwc_l(g_x_h_l),          # 大尺度全局语义
                ), dim=1)
            )
        )
        x_h_attn = x_h_attn.view(b, c, h_, 1)            # 重塑为高度方向注意力图

        # ================== 宽度方向注意力生成 ==================
        x_w_attn = self.sa_gate(
            self.norm_w(
                torch.cat((
                    self.local_dwc(l_x_w),               # 局部语义分支
                    self.global_dwc_s(g_x_w_s),          # 小尺度全局语义
                    self.global_dwc_m(g_x_w_m),          # 中尺度全局语义
                    self.global_dwc_l(g_x_w_l),          # 大尺度全局语义
                ), dim=1)
            )
        )
        x_w_attn = x_w_attn.view(b, c, 1, w_)            # 重塑为宽度方向注意力图

        # ================== 空间注意力融合 ==================
        x = x * x_h_attn * x_w_attn                      # 高度 × 宽度双向空间重标定

        return x                                        # 输出增强后的特征


if __name__ == '__main__':
    model = Shareable_Multi_Semantic_Spatial_Attention(dim=32)  # 初始化模块
    input = torch.randn(1, 32, 50, 50)                          # 构造输入特征
    output = model(input)                                       # 前向传播
    print(f'Input size: {input.size()}')                        # 输入尺寸
    print(f'Output size: {output.size()}')                      # 输出尺寸

```



# 55、Mask Attention 模块

论文《MaskAttn-UNet: A Mask Attention-Driven Framework for Universal Low-Resolution Image Segmentation》

### 1、作用

Mask Attention 模块旨在在低分辨率图像分割场景下，有效抑制无关背景区域，突出与目标分割最相关的空间位置，从而在分辨率受限、细节缺失的条件下提升分割精度与边界质量。



### 2、机制

Mask Attention 模块基于自注意力机制引入可学习的空间掩码（Mask）。 在计算注意力权重时，掩码直接参与注意力矩阵的构建，对不重要区域进行显式抑制，对关键区域进行增强。该模块通过“Masked Self-Attention + 残差前馈网络”的形式，在保持原始特征信息的同时，引入受控的全局上下文建模能力，并可在 U-Net 编码器与解码器各层多尺度嵌入。
...
1、**面向低分辨率任务设计**：
 通过掩码约束注意力范围，避免全局注意力在低分辨率条件下引入过多噪声信息，特别适合 128×128 及更低分辨率输入。

2、**兼顾局部与全局建模**：
 在保留 U-Net 卷积结构局部归纳偏置的同时，引入受控的长程依赖建模能力，实现局部细节与全局上下文的平衡。

3、**比全自注意力更稳定**：
 相较于直接使用自注意力或 Transformer，Mask Attention 通过显式掩码减少无效特征交互，更有利于边界保持与实例分离。

4、**可插拔性强**：
 Mask Attention 模块可作为独立注意力单元嵌入编码器、解码器或二者同时使用，适配多种 U-Net 及其变体结构。
...
import torch                              # PyTorch 主库
import torch.nn as nn                     # 神经网络模块
import torch.nn.functional as F           # 常用函数接口（Softmax 等）
...
class MaskAttention(nn.Module):
    def __init__(self, channels, size):
        ...
        self.size = size                  # 输入特征的空间尺寸（H, W）

        self.query = nn.Linear(channels, channels)  # 查询向量映射层 Q
        self.key = nn.Linear(channels, channels)    # 键向量映射层 K
        self.value = nn.Linear(channels, channels)  # 值向量映射层 V

        self.mask = None                  # 掩码初始化为空
        self.norm = nn.LayerNorm([channels])  # 层归一化，用于稳定训练

    def forward(self, x):
        batch_size, channels, height, width = x.size()  # 输入尺寸 [B, C, H, W]

        if channels != self.channels:     # 校验通道数一致性
            raise ValueError("Input channel size does not match initialized channel size.")

        # -------- Step 1: 特征展平 --------
        x = x.view(batch_size, channels, height * width).permute(0, 2, 1)
        # [B, C, H*W] → [B, H*W, C]，每个空间位置作为一个 token

        # -------- Step 2: Q K V 计算 --------
        Q = self.query(x)                 # 查询向量 Q [B, H*W, C]
        K = self.key(x)                   # 键向量 K [B, H*W, C]
        V = self.value(x)                 # 值向量 V [B, H*W, C]

        # -------- Step 3: 注意力分数计算 --------
        scores = torch.matmul(Q, K.transpose(-2, -1))  # 点积注意力 [B, H*W, H*W]
        scores = scores / (self.channels ** 0.5)       # 缩放，防止数值过大
        ...
        if self.mask is None or self.mask.size(-1) != height * width:
            ...
            binary_mask = binary_mask.view(batch_size, -1)  # 展平为 [B, H*W]

            processed_mask = torch.where(
                binary_mask > 0.5,
                torch.tensor(0.0, device=x.device),         # 允许关注的位置
                torch.tensor(-float('inf'), device=x.device) # 屏蔽的位置
            )

            self.mask = processed_mask.unsqueeze(1).expand(
                -1, height * width, -1
            )                                               # 扩展为 [B, H*W, H*W]

        # -------- Step 5: 加入掩码并计算注意力权重 --------
        scores = scores + self.mask                         # 应用掩码
        attention_weights = F.softmax(scores, dim=-1)       # Softmax 得到注意力权重

        # -------- Step 6: 注意力加权输出 --------
        attention_output = torch.matmul(attention_weights, V)  # 加权求和 [B, H*W, C]

        # -------- Step 7: 残差连接与归一化 --------
        attention_output = attention_output + x             # 残差连接
        attention_output = self.norm(attention_output)      # LayerNorm 稳定特征分布
...
if __name__ == "__main__":
    input_tensor = torch.randn(1, 32, 50, 50)               # 构造输入特征
    model = MaskAttention(channels=32, size=(50, 50))       # 初始化模块
    output = model(input_tensor)                            # 前向传播
    print(f'Input size: {input_tensor.size()}')             # 输入尺寸
    print(f'Output size: {output.size()}')                  # 输出尺寸
...
论文《Flora-NET: Integrating dual coordinate attention with adaptive kernel based convolution network for medicinal flower identification》

### 1、作用

Flora-NET 旨在解决药用花卉分类中类内差异大、类间差异小以及结构高度相似的问题，通过强化花瓣纹理、花朵结构与空间依赖特征建模，提升模型对细粒度药用花卉类别的判别能力，从而服务于药用识别、生物多样性保护及相关产业应用。



### 2、机制

Flora-NET 通过注意力引导 + 空间自适应算子进行特征精炼，核心由两个模块组成： DCAFE（Dual Coordinate Attention Feature Extraction）模块在通道注意力中引入坐标信息，采用平均池化与最大池化并行的坐标注意力机制，增强花瓣形态与方向性结构特征表达； Inv-FR（Involution-Based Feature Refinement）模块利用串联的 Involution 层，通过位置相关、通道共享的自适应核，对空间结构进行精细建模，从而弥补传统卷积在复杂花卉结构建模上的不足。
...
1、**面向细粒度花卉结构建模**：
 针对花瓣纹理、形态变化与空间排列等关键细节进行强化，适合高视觉相似度类别的区分任务。

2、**坐标注意力与 Involution 协同**：
 通过坐标注意力捕获方向与位置信息，再由 Involution 强化空间依赖，实现“先定位、再细化”的特征精炼流程。

3、**突破传统卷积局限**：
 Involution 采用自适应空间核，提升对复杂结构与非刚性形态的建模能力，优于固定卷积核的表达方式。

4、**稳定且可验证的性能提升**：
 通过消融实验、超参数调优与 k 折交叉验证，验证各模块对性能提升的独立与协同贡献，整体精度显著优于现有方法。
...
class CoordAttMeanMax(nn.Module):
    def __init__(self, inp, oup, groups=32):
        ...
        # ================== 坐标方向池化（Mean / Max） ==================
        self.pool_h_mean = nn.AdaptiveAvgPool2d((None, 1))  # 高度方向均值池化
        self.pool_w_mean = nn.AdaptiveAvgPool2d((1, None))  # 宽度方向均值池化
        self.pool_h_max = nn.AdaptiveMaxPool2d((None, 1))   # 高度方向最大池化
        self.pool_w_max = nn.AdaptiveMaxPool2d((1, None))   # 宽度方向最大池化

        mip = max(8, inp // groups)                         # 中间压缩通道数（保证最小维度）

        # ================== Mean Pooling 分支 ==================
        self.conv1_mean = nn.Conv2d(inp, mip, kernel_size=1, stride=1, padding=0)  # 通道压缩
        self.bn1_mean = nn.BatchNorm2d(mip)                # 批归一化
        self.conv2_mean = nn.Conv2d(mip, oup, kernel_size=1, stride=1, padding=0)  # 通道恢复

        # ================== Max Pooling 分支 ==================
        self.conv1_max = nn.Conv2d(inp, mip, kernel_size=1, stride=1, padding=0)   # 通道压缩
        self.bn1_max = nn.BatchNorm2d(mip)                 # 批归一化
        self.conv2_max = nn.Conv2d(mip, oup, kernel_size=1, stride=1, padding=0)   # 通道恢复

        self.relu = nn.ReLU(inplace=True)                  # 非线性激活函数

    def forward(self, x):
        identity = x                                      # 保存残差分支
        n, c, h, w = x.size()                             # 输入尺寸 [B, C, H, W]

        # ================== Mean Pooling 分支 ==================
        x_h_mean = self.pool_h_mean(x)                    # 高度方向均值特征 [B, C, H, 1]
        x_w_mean = self.pool_w_mean(x).permute(0, 1, 3, 2) # 宽度方向均值特征 [B, C, W, 1]
        y_mean = torch.cat([x_h_mean, x_w_mean], dim=2)   # 拼接高度和宽度特征
        y_mean = self.conv1_mean(y_mean)                  # 通道压缩
        y_mean = self.bn1_mean(y_mean)                    # 归一化
        y_mean = self.relu(y_mean)                        # 激活
        x_h_mean, x_w_mean = torch.split(y_mean, [h, w], dim=2)  # 拆分为 H / W 两个方向
        x_w_mean = x_w_mean.permute(0, 1, 3, 2)            # 恢复宽度方向维度

        # ================== Max Pooling 分支 ==================
        x_h_max = self.pool_h_max(x)                      # 高度方向最大响应 [B, C, H, 1]
        x_w_max = self.pool_w_max(x).permute(0, 1, 3, 2)   # 宽度方向最大响应 [B, C, W, 1]
        y_max = torch.cat([x_h_max, x_w_max], dim=2)      # 拼接高度和宽度特征
        y_max = self.conv1_max(y_max)                     # 通道压缩
        y_max = self.bn1_max(y_max)                       # 归一化
        y_max = self.relu(y_max)                          # 激活
        x_h_max, x_w_max = torch.split(y_max, [h, w], dim=2)  # 拆分为 H / W 两个方向
        x_w_max = x_w_max.permute(0, 1, 3, 2)              # 恢复宽度方向维度

        # ================== 生成坐标注意力 ==================
        x_h_mean = self.conv2_mean(x_h_mean).sigmoid()    # 高度方向均值注意力
        x_w_mean = self.conv2_mean(x_w_mean).sigmoid()    # 宽度方向均值注意力
        x_h_max = self.conv2_max(x_h_max).sigmoid()       # 高度方向最大值注意力
        x_w_max = self.conv2_max(x_w_max).sigmoid()       # 宽度方向最大值注意力
        ...
        # ================== 注意力融合 ==================
        attention_mean = identity * x_w_mean * x_h_mean   # Mean 分支的坐标注意力增强
        attention_max = identity * x_w_max * x_h_max      # Max 分支的坐标注意力增强

        return attention_mean + attention_max              # 融合两种注意力结果
...
if __name__ == "__main__":
    ...
    x = torch.randn(batch_size, channels, height, width).cuda()  # 构造输入张量

    model = CoordAttMeanMax(inp=channels, oup=channels).cuda()    # 实例化模块
...
论文《Dual Selective Fusion Transformer Network for Hyperspectral Image Classification》

### 1、作用

DSFormer 旨在解决高光谱图像分类中固定感受野不匹配目标尺度、无效 token 干扰上下文建模的问题，通过对空间—光谱特征进行选择性融合，使模型能够在合适的感受野内聚焦最具判别力的上下文信息，从而提升复杂地物场景下的分类精度与稳定性。



### 2、机制

DSFormer 通过双重选择性融合策略实现空间—光谱联合建模，核心由两类 Transformer 模块构成：KSFTB（Kernel Selective Fusion Transformer Block）在特征提取阶段引入多尺度卷积核，通过空间—光谱选择机制自适应融合不同感受野的特征，学习最合适的上下文尺度；TSFTB（Token Selective Fusion Transformer Block）在自注意力阶段对 token 进行筛选，仅保留高相关性的空间—光谱 token 参与注意力计算，从而减少冗余信息干扰并强化关键上下文建模能力。
...
1、**自适应感受野选择**：
 通过多尺度卷积核与选择性融合机制，使不同地物类别能够在最合适的上下文尺度下被建模，克服固定感受野带来的误判问题。

2、**抑制无效注意力干扰**：
 TSFTB 通过 token 选择机制减少无关区域参与自注意力计算，提升空间—光谱特征的判别性与解释性。

3、**空间—光谱联合建模能力强**：
 在 Transformer 框架下同时建模空间结构与光谱依赖关系，避免仅关注单一维度信息造成的特征缺失。

4、**适配复杂遥感场景**：
 在多种高光谱基准数据集上表现稳定，对多类别、尺度差异显著的复杂地物具有良好泛化能力。
...
class KernelSelectiveFusionAttention(nn.Module):
    def __init__(self, dim, r=16, L=32):
        super().__init__()
        d = max(dim // r, L)          # 通道压缩后的维度，设下限避免过小
        ...
        self.conv_spatial = nn.Conv2d(
            ...
            groups=dim,
            dilation=2
        )                              # 5×5 空洞深度卷积，扩大感受野，建模更大范围上下文

        # ================== 通道压缩层 ==================
        self.conv1 = nn.Conv2d(dim, dim // 2, 1)  # 分支1 通道压缩
        self.conv2 = nn.Conv2d(dim, dim // 2, 1)  # 分支2 通道压缩

        # ================== 空间注意力融合层 ==================
        self.conv_squeeze = nn.Conv2d(
            2, 2, 7, padding=3
        )                              # 融合平均 / 最大池化空间信息（定义但未显式使用）
        self.conv = nn.Conv2d(
            dim // 2, dim, 1
        )                              # 将融合特征映射回原通道数
        ...
        self.global_maxpool = nn.AdaptiveMaxPool2d(1) # 全局最大池化（定义但未显式使用）

        self.fc1 = nn.Sequential(
            nn.Conv2d(dim, d, 1, bias=False),        # 通道压缩
            nn.BatchNorm2d(d),
            nn.ReLU(inplace=True)
        )
        self.fc2 = nn.Conv2d(
            d, dim, 1, 1, bias=False
        )                              # 生成通道注意力权重

        self.softmax = nn.Softmax(dim=1)  # 在分支维度上做 Softmax，实现选择性加权
    ...
    def forward(self, x):
        ...
        # ================== 双分支空间特征提取 ==================
        attn1 = self.conv0(x)          # 分支1：3×3 深度卷积
        attn2 = self.conv_spatial(attn1) # 分支2：5×5 空洞深度卷积

        # ================== 通道压缩 ==================
        attn1 = self.conv1(attn1)      # 分支1 降通道
        attn2 = self.conv2(attn2)      # 分支2 降通道
        ...
        agg = torch.cat([avg_attn, max_attn], dim=1)      # 聚合空间描述

        # ================== 通道注意力计算（下半部分） ==================
        ch_attn1 = self.global_pool(attn)         # 全局平均池化
        z = self.fc1(ch_attn1)                    # 通道压缩
        a_b = self.fc2(z)                         # 通道恢复

        a_b = a_b.reshape(batch_size, 2, dim // 2, -1)
        a_b = self.softmax(a_b)                   # 分支间权重归一化

        a1, a2 = a_b.chunk(2, dim=1)              # 拆分两个分支的权重
        a1 = a1.reshape(batch_size, dim // 2, 1, 1)
        a2 = a2.reshape(batch_size, dim // 2, 1, 1)

        w1 = a1 * agg[:, 0, :, :].unsqueeze(1)    # 分支1 的空间加权
        w2 = a2 * agg[:, 0, :, :].unsqueeze(1)    # 分支2 的空间加权

        # ================== 特征融合与输出 ==================
        attn = attn1 * w1 + attn2 * w2             # 选择性融合双分支特征
        attn = self.conv(attn).sigmoid()           # 生成空间注意力图

        return x * attn                            # 对输入特征进行空间重标定


if __name__ == '__main__':
    x = torch.randn(2, 32, 50, 50)                 # 构造输入张量
    model = KernelSelectiveFusionAttention(dim=32) # 实例化模块
    output = model(x)                              # 前向传播
...
论文《FMNet: Frequency-Assisted Mamba-Like Linear Attention Network for Camouflaged Object Detection》

### 1、作用

FMNet 旨在解决伪装目标与背景高度相似、边界模糊、尺度变化大导致的检测困难问题，通过引入频域建模与高效线性注意力机制，增强模型对全局语义与隐藏目标的感知能力，在保证精度的同时显著降低计算复杂度。



### 2、机制

FMNet 通过频域特征 + Mamba 类线性注意力 + 多尺度融合进行整体建模：首先，PFAE（Pyramidal Frequency Attention Extraction）模块在频域中引入注意力机制，以多尺度方式提取高频与全局语义信息；随后，MFM（Multi-scale Frequency-Assisted Mamba-Like Linear Attention）模块将频域信息与 Mamba-like 线性注意力结合，在多尺度结构中高效建模长程依赖；最后，FRD（Frequency Reverse Decoder）通过频域反向注意力与跨层特征聚合，逐级恢复空间细节并强化伪装区域响应。
...
1、**频域辅助的全局建模能力**：
 利用频域特征天然的全局感知优势，有效缓解伪装目标与复杂背景之间的语义混淆问题。

2、**高效替代 Transformer 注意力**：
 基于 Mamba-like 线性注意力，在保持长程依赖建模能力的同时，大幅降低参数量与 FLOPs，提升实际可用性。

3、**多尺度频域—空间协同设计**：
 通过金字塔频域注意力与多尺度线性注意力融合，兼顾大目标、小目标及遮挡场景下的检测稳定性。

4、**性能与效率兼顾**：
 在多个 COD 基准数据集上实现优于主流方法的检测性能，同时具备明显的参数与计算效率优势，适合实际部署。
...
from einops import rearrange, repeat            # 张量重排工具
from torch.nn import Softmax                   # Softmax 函数


# ================== 复数 Softmax 归一化 ==================
def custom_complex_normalization(input_tensor, dim=-1):
    real_part = input_tensor.real              # 提取复数的实部
    imag_part = input_tensor.imag              # 提取复数的虚部

    norm_real = F.softmax(real_part, dim=dim)  # 对实部做 Softmax
    norm_imag = F.softmax(imag_part, dim=dim)  # 对虚部做 Softmax
...
class FrequencyAttention(nn.Module):
    def __init__(self, in_dim):
        ...
        down_dim = in_dim // 2                  # 通道降维，降低计算复杂度

        # ================== 空间卷积分支 ==================
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_dim, down_dim, kernel_size=1),  # 1×1 卷积降通道
            nn.BatchNorm2d(down_dim),
            nn.ReLU(True)
        )
        ...
        self.value_conv2 = nn.Conv2d(
            in_channels=down_dim,
            out_channels=down_dim,
            kernel_size=1
        )                                       # Value 映射

        self.gamma2 = nn.Parameter(torch.zeros(1))  # 可学习缩放参数
        ...
        self.softmax = Softmax(dim=-1)          # Softmax 操作
        self.norm = nn.BatchNorm2d(down_dim)    # 批归一化
        self.relu = nn.ReLU(True)               # 激活函数
        self.num_heads = 8                      # 多头注意力头数

    def forward(self, x):
        conv2 = self.conv2(x)                   # 空间特征提取
        b, c, h, w = conv2.shape                # 特征尺寸

        # ================== FFT 变换到频域 ==================
        q_f_2 = torch.fft.fft2(conv2.float())   # Query 频域表示
        k_f_2 = torch.fft.fft2(conv2.float())   # Key 频域表示
        v_f_2 = torch.fft.fft2(conv2.float())   # Value 频域表示
        tepqkv = torch.fft.fft2(conv2.float())  # 频域特征备份（用于幅值调制）

        # ================== 多头频域注意力重排 ==================
        q_f_2 = rearrange(
            q_f_2, 'b (head c) h w -> b head c (h w)', head=self.num_heads
        )
        k_f_2 = rearrange(
            k_f_2, 'b (head c) h w -> b head c (h w)', head=self.num_heads
        )
        v_f_2 = rearrange(
            v_f_2, 'b (head c) h w -> b head c (h w)', head=self.num_heads
        )

        # ================== 频域注意力计算 ==================
        q_f_2 = torch.nn.functional.normalize(q_f_2, dim=-1)  # 归一化 Query
        k_f_2 = torch.nn.functional.normalize(k_f_2, dim=-1)  # 归一化 Key

        attn_f_2 = (q_f_2 @ k_f_2.transpose(-2, -1)) * self.temperature
        # 复数点积注意力

        attn_f_2 = custom_complex_normalization(attn_f_2, dim=-1)
        # 对复数注意力进行 Softmax 归一化

        out_f_2 = torch.abs(
            torch.fft.ifft2(attn_f_2 @ v_f_2)
        )                                       # 反 FFT 回到空间域并取幅值
        ...
        # ================== 频域幅值调制分支 ==================
        out_f_l_2 = torch.abs(
            torch.fft.ifft2(self.weight(tepqkv.real) * tepqkv)
        )                                       # 幅值加权的频域重建

        # ================== 特征融合 ==================
        out_2 = torch.cat((out_f_2, out_f_l_2), 1)  # 拼接两种频域特征
        F_2 = torch.add(out_2, x)                   # 残差连接

        return F_2


if __name__ == "__main__":
    input_tensor = torch.randn(1, 32, 256, 256)     # 构造输入张量

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    model = FrequencyAttention(in_dim=32).to(device)  # 实例化模型
...
论文《CAF-YOLO:ARobust Framework for Multi-Scale Lesion Detection in Biomedical Imagery》

### 1、作用

CAF-YOLO 旨在解决医学影像中微小病灶（如血小板、微小肺结节）难以检测的问题，通过同时增强全局依赖建模能力与多尺度特征聚合能力，提高模型对小尺度、模糊及遮挡目标的检测精度与定位可靠性。



### 2、机制

CAF-YOLO 在 YOLOv8 框架基础上引入 CAFBlock，其核心由两个模块组成：ACFM（Attention and Convolution Fusion Module）通过并行的全局注意力分支与局部卷积分支，将自注意力的长程依赖建模能力与卷积的局部结构建模能力进行融合，实现全局—局部特征协同增强；MSNN（Multi-Scale Neural Network）用于弥补 Transformer 中 FFN 单尺度特征聚合的不足，通过多尺度空洞卷积与深度可分离卷积并行提取特征，并利用门控机制完成多尺度信息融合，从而提升对不同尺寸病灶的表达能力。
...
1、**全局与局部特征融合建模**：
 通过 ACFM 将注意力机制与卷积算子有效结合，兼顾长程依赖与局部空间结构，特别适合医学图像中形态差异细微的目标。

2、**强化微小目标检测能力**：
 MSNN 提供多尺度特征聚合能力，显著提升对极小病灶与尺度变化目标的感知效果。

3、**兼容 YOLO 实时检测框架**：
 在保持 YOLOv8 高效推理特性的同时，引入 Transformer 思想，兼顾精度与效率。

4、**良好的泛化性与可验证性**：
 在 BCCD 与 LUNA16 等医学数据集上取得稳定提升，并通过消融实验验证各模块的独立与协同作用。
...
class ACFMAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape                                 # 输入尺寸 [B, C, H, W]

        # ================== QKV 特征构建 ==================
        x = x.unsqueeze(2)                                   # 扩展维度 → [B, C, 1, H, W]
        qkv = self.qkv_dwconv(self.qkv(x))                   # QKV + 局部 3D 卷积
        qkv = qkv.squeeze(2)                                 # 去掉深度维度 → [B, 3C, H, W]

        # ================== 局部卷积分支（ACFM Local） ==================
        f_conv = qkv.permute(0, 2, 3, 1)                     # [B, H, W, 3C]
        f_all = qkv.reshape(
            f_conv.shape[0], h * w,
            3 * self.num_heads, -1
        ).permute(0, 2, 1, 3)                                 # [B, 3H, HW, C/H]

        f_all = self.fc(f_all.unsqueeze(2))                  # 生成局部卷积权重
        f_all = f_all.squeeze(2)

        # -------- Local Convolution --------
        f_conv = f_all.permute(0, 3, 1, 2).reshape(
            x.shape[0],
            9 * x.shape[1] // self.num_heads,
            h, w
        )
        f_conv = f_conv.unsqueeze(2)
        out_conv = self.dep_conv(f_conv)                     # 局部上下文特征
        out_conv = out_conv.squeeze(2)

        # ================== 全局自注意力分支（Global SA） ==================
        q, k, v = qkv.chunk(3, dim=1)                        # 拆分 Q / K / V
        ...
        q = torch.nn.functional.normalize(q, dim=-1)         # Q 归一化
        k = torch.nn.functional.normalize(k, dim=-1)         # K 归一化

        attn = (q @ k.transpose(-2, -1)) * self.temperature  # 注意力权重计算
...
if __name__ == "__main__":
    ...
    x = torch.randn(batch_size, channels, height, width)     # 构造输入张量
...
论文《PlainUSR: Chasing Faster ConvNet for Efficient Super-Resolution》

### 1、作用

PlainUSR 旨在解决高性能超分辨率模型在实际部署中推理延迟高、内存访问开销大的问题，通过针对卷积模块、注意力机制与骨干结构的系统性重构，在保持甚至提升重建质量的同时，大幅降低推理延迟，实现真正可部署的高效超分辨率网络。



### 2、机制

PlainUSR 从三个关键层面进行整体优化：在卷积层面，引入 RepMBConv，通过重参数化技术将训练阶段的复杂 MBConv 结构等价折叠为推理阶段的单一标准卷积，从而减少内存访问并显著降低延迟；在注意力层面，提出 LIA（Local Importance-based Attention），利用局部重要性估计与门控机制，在近似二阶信息交互复杂度下实现高效空间注意力建模；在骨干结构层面，设计 PlainU-Net，通过通道维度的分裂与拼接构建 U 型特征流，使训练阶段具备层次表达能力，而推理阶段保持纯 VGG-style 的高效前向计算。
...
1、**以“延迟”为核心目标的系统级设计**：
 从卷积、注意力到骨干结构同时优化，避免仅依赖轻量化模块却忽视内存访问瓶颈的问题。

2、**重参数化带来的推理友好性**：
 RepMBConv 在不牺牲性能的前提下，将复杂结构压缩为单一卷积算子，显著提升实际硬件上的运行速度。

3、**低成本高阶注意力建模**：
 LIA 在接近一阶注意力计算量的条件下实现近似二阶空间交互，兼顾效果与效率。

4、**训练复杂、推理简洁的结构优势**：
 PlainU-Net 在训练阶段引入 U 型层次表达，在推理阶段退化为简单直连结构，兼顾优化稳定性与部署效率。
...
class SoftPooling2D(torch.nn.Module):
    def __init__(self, kernel_size, stride=None, padding=0):
        ...
        # 使用平均池化作为基础操作，不将 padding 区域计入统计
    ...
    def forward(self, x):
        x_exp = torch.exp(x)               # 对输入特征做指数映射，增强大响应
        x_exp_pool = self.avgpool(x_exp)   # 对指数特征进行平均池化
        x = self.avgpool(x_exp * x)        # 指数加权后的特征再做平均池化
        return x / x_exp_pool              # 归一化，得到加权平均结果
...
class LocalAttention(nn.Module):
    def __init__(self, channels, f=16):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(channels, f, 1),     # 1×1 卷积压缩通道数
            SoftPooling2D(7, stride=3),    # Soft Pooling，突出显著区域
            nn.Conv2d(f, f, kernel_size=3, stride=2, padding=1),  # 下采样并提取局部结构
            nn.Conv2d(f, channels, 3, padding=1),  # 恢复到原通道数
            nn.Sigmoid(),                  # 生成注意力权重（0~1）
        )

        """
        门控机制说明：
        为避免步长卷积和插值操作带来的伪影，仅利用输入的局部显著性
        对注意力权重进行再约束，提高稳定性与鲁棒性。
        """
    ...
    def forward(self, x):
        g = self.gate(x[:, :1].clone())    # 使用输入的第一个通道生成门控信号
        ...
        return x * w * g                   # 输入特征 × 注意力权重 × 门控信号


if __name__ == "__main__":
    input = torch.randn(1, 32, 64, 64)     # 构造随机输入张量
    LA = LocalAttention(32)                # 初始化局部注意力模块
    output = LA(input)                     # 前向传播
    print('input_size:', input.size())     # 输入尺寸
    print('output_size:', output.size())   # 输出尺寸
...
HVI + CIDNet 旨在解决低光照图像增强中亮度与颜色高度耦合、红色断裂噪声与黑色平面噪声严重的问题，通过构建专用于低光照任务的新型颜色空间，并配合颜色—强度解耦网络，实现更加自然、稳定且无色偏的低光照图像增强。



### 2、机制

该方法由 HVI 颜色空间设计 与 CIDNet 解耦增强网络 两部分协同构成：HVI 颜色空间在 HSV 基础上对 Hue–Saturation 平面进行极化处理，消除红色在色相轴上的不连续性，同时引入可学习的强度塌缩函数 Ck，对低亮度区域进行自适应压缩，从而缓解黑色噪声问题；CIDNet 在 HVI 空间中采用双分支结构，分别建模 HV 色彩信息与强度信息，并通过跨分支注意力实现颜色去噪与亮度增强的协同优化，最终通过感知逆 HVI 映射恢复至 sRGB 空间。
...
1、**从颜色空间层面解决低光照伪影问题**：
 不同于仅在网络结构上修补，HVI 直接从颜色表示层面消除 HSV 中固有的红色断裂与黑平面噪声。

2、**亮度与颜色高度解耦**：
 通过 HVI 设计与双分支网络，使亮度增强不再引发颜色偏移，显著提升视觉自然度。

3、**可学习的低光照建模能力**：
 引入可训练参数 k，使低光照区域的压缩程度能够自适应不同数据集与成像条件。

4、**良好的通用性与可插拔性**：
 HVI 颜色空间可作为前端模块直接嵌入多种现有 LLIE 方法中，在不改动网络结构的情况下带来稳定性能提升。
...
# ================== 自定义 LayerNorm（支持 NCHW / NHWC） ==================
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_first"):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(normalized_shape))  # 可学习缩放参数 γ
        self.bias = nn.Parameter(torch.zeros(normalized_shape))   # 可学习偏置参数 β
        self.eps = eps                                            # 数值稳定项
        self.data_format = data_format                            # 数据格式标志
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            # NHWC 格式，直接调用 PyTorch LayerNorm
            return F.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)

        elif self.data_format == "channels_first":
            # NCHW 格式，手动实现 LayerNorm
            u = x.mean(1, keepdim=True)                            # 通道维度均值
            s = (x - u).pow(2).mean(1, keepdim=True)               # 通道维度方差
...
class IEL(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2.66, bias=False):
        ...
        hidden_features = int(dim * ffn_expansion_factor)          # 隐藏层通道数
    ...
    def forward(self, x):
        x = self.project_in(x)                                     # 通道扩展
        x1, x2 = self.dwconv(x).chunk(2, dim=1)                    # 拆分为两路

        x1 = self.Tanh(self.dwconv1(x1)) + x1                      # 分支1 强度增强 + 残差
        x2 = self.Tanh(self.dwconv2(x2)) + x2                      # 分支2 强度增强 + 残差

        x = x1 * x2                                                # 双分支逐元素调制
        x = self.project_out(x)                                    # 投影回原通道
        return x
...
class CAB(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.q = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)     # Query 映射
        self.q_dwconv = nn.Conv2d(
            dim, dim, kernel_size=3, stride=1, padding=1,
            groups=dim, bias=bias
        )                                                          # Query 深度卷积

        self.kv = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias) # Key/Value 映射
    ...
    def forward(self, x, y):
        b, c, h, w = x.shape                                       # 输入尺寸

        q = self.q_dwconv(self.q(x))                               # 计算 Query
        kv = self.kv_dwconv(self.kv(y))                            # 计算 Key / Value
        k, v = kv.chunk(2, dim=1)                                  # 拆分 K 和 V

        q = rearrange(q, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        k = rearrange(k, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        v = rearrange(v, 'b (head c) h w -> b head c (h w)', head=self.num_heads)

        q = torch.nn.functional.normalize(q, dim=-1)               # Query 归一化
        k = torch.nn.functional.normalize(k, dim=-1)               # Key 归一化

        attn = (q @ k.transpose(-2, -1)) * self.temperature        # 注意力得分
        attn = nn.functional.softmax(attn, dim=-1)                 # Softmax 归一化
...
class LCA(nn.Module):
    def __init__(self, dim, num_heads, bias=False):
        ...
        self.gdfn = IEL(dim)                                        # 强度增强模块
        self.norm = LayerNorm(dim)                                  # 层归一化
        self.ffn = CAB(dim, num_heads, bias)                        # 交叉注意力模块

    def forward(self, x, y):
        x = x + self.ffn(self.norm(x), self.norm(y))                # Cross-Attention 残差
        x = self.gdfn(self.norm(x)) + x                             # 强度增强 + 残差
        return x


if __name__ == "__main__":
    module = LCA(dim=64, num_heads=8)                               # 实例化 LCA
    input_x = torch.randn(1, 64, 32, 32)                            # 输入特征 x
    input_y = torch.randn(1, 64, 32, 32)                            # 输入特征 y

    output_tensor = module(input_x, input_y)                        # 前向传播
...
论文《Efficient Attention-Sharing Information Distillation Transformer for Lightweight Single Image Super-Resolution》

### 1、作用

ASID 旨在解决Transformer 超分辨率模型计算复杂度高、参数量大、难以实际部署的问题，通过将信息蒸馏思想引入 Transformer 架构，并结合注意力共享机制，在极低参数量条件下实现高质量的单幅图像超分辨率重建。



### 2、机制

ASID 通过信息蒸馏结构 + 注意力共享 + 通道分裂协同降低计算开销： 在 Information Distillation Block（IDB）中，特征被逐级拆分为“精炼特征”和“粗特征”，仅将部分关键通道送入后续自注意力模块，减少高成本注意力计算；在空间自注意力中，引入 Attention-Sharing 机制，仅在首个 IDB 中计算注意力矩阵，后续 IDB 复用该矩阵，避免重复的相关性计算；同时结合 Channel-Split 策略，限制参与自注意力计算的通道数量，从而在保持长程依赖建模能力的同时显著降低参数量与 FLOPs。



### 3、独特优势

1、**将信息蒸馏成功迁移到 Transformer**：
 不同于传统 CNN 蒸馏，ASID 针对自注意力高计算特性重新设计蒸馏路径，解决 Transformer 轻量化难题。

2、**注意力矩阵复用，显著降低复杂度**：
 通过跨模块共享空间注意力矩阵，避免多层 Transformer 重复计算高成本相关性映射。

3、**长程建模能力不缩水**：
 在减少通道数与注意力计算的前提下，仍能保留 Transformer 对全局依赖的建模优势。

4、**极致参数效率**：
 在约 300K 参数规模下实现与主流 Transformer SR 方法相当甚至更优的重建效果，具备实际部署价值。
...
```python
import torch                                   # PyTorch 主库
from torch import nn, einsum                  # 神经网络模块与爱因斯坦求和
from einops import rearrange                  # 张量重排工具
import torch.nn.functional as F               # 常用函数接口


# ================== 自定义 LayerNorm 的 Autograd 实现（NCHW） ==================
class LayerNormFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias, eps):
        ...
        mu = x.mean(1, keepdim=True)           # 按通道计算均值
        var = (x - mu).pow(2).mean(1, keepdim=True)  # 按通道计算方差
        y = (x - mu) / (var + eps).sqrt()      # 归一化

        ctx.save_for_backward(y, var, weight)  # 保存反向传播所需变量
    ...
    def backward(ctx, grad_output):
        ...
        y, var, weight = ctx.saved_variables   # 取出缓存变量
        g = grad_output * weight.view(1, C, 1, 1)

        mean_g = g.mean(dim=1, keepdim=True)   # 梯度均值
        mean_gy = (g * y).mean(dim=1, keepdim=True)

        gx = 1. / torch.sqrt(var + eps) * (g - y * mean_gy - mean_g)
        return (
            gx,
            (grad_output * y).sum(dim=3).sum(dim=2).sum(dim=0),   # weight 梯度
            grad_output.sum(dim=3).sum(dim=2).sum(dim=0),         # bias 梯度
            None
        )
...
class LayerNorm2d(nn.Module):
    def __init__(self, channels, eps=1e-6):
        ...
        self.register_parameter('weight', nn.Parameter(torch.ones(channels)))  # γ
        self.register_parameter('bias', nn.Parameter(torch.zeros(channels)))   # β
        self.eps = eps
...
# ================== Gated Convolutional FeedForward ==================
class Gated_Conv_FeedForward(nn.Module):
    def __init__(self, dim, mult=1, bias=False, dropout=0.):
        super().__init__()
        hidden_features = int(dim * mult)      # 隐藏层通道数

        self.project_in = nn.Conv2d(
            dim, hidden_features * 2,
...
class SelfAttentionA(nn.Module):
    def __init__(
        ...
        self.heads = dim // dim_head            # 注意力头数
        self.scale = dim_head ** -0.5           # 缩放因子
        self.with_pe = True

        self.to_qkv = nn.Linear(dim, dim * 3, bias=False)  # QKV 映射
    ...
    def forward(self, x):
        ...
        q = q * self.scale                    # 缩放 Query
        sim = einsum('b h i d, b h j d -> b h i j', q, k)  # 相似度计算

        if self.with_pe:
            bias = self.rel_pos_bias(self.rel_pos_indices)
            sim = sim + rearrange(bias, 'i j h -> h i j')

        attn = self.attend(sim)               # 注意力权重
        out = einsum('b h i j, b h j d -> b h i d', attn, v)  # 加权求和

        out = rearrange(
            out, 'b h (w1 w2) d -> b w1 w2 (h d)',
...
class SLA_A(nn.Module):
    def __init__(self, channel_num=64, depth=0, bias=True, ffn_bias=True, window_size=8, with_pe=False, dropout=0.0):
        ...
        self.w = 8
        self.norm = nn.LayerNorm(channel_num)          # Pre-Norm
        ...
        self.cnorm = LayerNorm2d(channel_num)          # Conv LayerNorm
    ...
    def forward(self, x):
        ...
        x, a = self.attn(self.norm(x_))                # Window Attention
        x = rearrange(x + x_, 'b x y w1 w2 d -> b d (x w1) (y w2)')
        x = self.gfn(self.cnorm(x)) + x                # FFN + 残差
        return x, a
...
if __name__ == "__main__":
    ...
    x = torch.randn(batch_size, channels, height, width)  # 输入特征
...
论文《An Adaptive Dual-Supervised Cross-Deep Dependency Network for Pixel-Wise Classification》

### 1、作用

ADCD-Net 旨在解决多源遥感数据（MS 与 PAN）融合中语义差异大、特征不一致、冗余严重的问题，通过跨模态语义对齐与深层依赖建模，重建更加丰富且一致的融合特征，从而提升下游语义分类与判别性能。



### 2、机制

ADCD-Net 采用两阶段自适应双监督训练框架： Stage I 引入语义感知自监督策略（SPS），在无显式标签约束下学习跨模态深层特征，通过语义感知损失缩小 MS 与 PAN 之间的语义鸿沟，同时保留各自的非冗余特征；Stage II 引入深时序 Mamba 模块（DTM-Module），对网络各层输出进行交互式建模，使多层深特征参与强化学习，增强语义信息的层间传递与依赖建模；在两阶段训练过程中，进一步引入 DIA-Module（Deformable Interactive Attention），通过可变形注意力抑制通道冗余，同时强化空间位置记忆与二维特征判别能力。
...
1、**两阶段语义对齐与强化设计**：
 先消除跨模态语义差异，再进行深层依赖强化，符合多源融合的实际学习顺序。

2、**Mamba 驱动的深层交互建模**：
 DTM-Module 利用线性复杂度的时序建模能力，实现多层特征的高效依赖学习，优于传统自注意力结构。

3、**通道—空间协同去冗余**：
 DIA-Module 在抑制冗余通道的同时增强空间位置感知，提高融合特征的判别性与稳定性。

4、**良好的跨数据集泛化能力**：
 在多套遥感数据集上的对比与迁移实验中表现稳定，适合多源遥感融合与分类任务。
...
class DeformableInteractiveAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.distortionmode:
            ...
            out = torch.cat(
                [d_avg_out * max_out, d_max_out * avg_out],
                dim=1
            )
            # 使用调制因子对统计特征进行交互增强

        # ================== 注意力掩码生成 ==================
        out = self.conv(out)                     # 空间注意力映射
        mask = self.sigmoid(self.upsample(out))  # 上采样并归一化为 [0,1]

        # ================== 注意力加权 ==================
        att_out = x * mask                       # 对输入特征进行空间加权
        return F.relu(att_out)                   # ReLU 输出增强后的特征
...
if __name__ == '__main__':
    ...
    model = DeformableInteractiveAttention(
        stride=1,
        distortionmode=True
    ).cuda()
...
论文《TOKEN STATISTICS TRANSFORMER: LINEAR-TIME ATTENTION VIA VARIATIONAL RATE REDUCTION》

### 1、作用

TOST 旨在解决Transformer 自注意力在 token 数量较大时计算与显存开销呈二次增长的问题，通过彻底摆脱 token 两两相似度计算，引入基于 token 统计量的注意力机制，在保持全局建模能力的同时，将注意力计算复杂度降为线性级别。



### 2、机制

TOST 的核心是 TSSA（Token Statistics Self-Attention），其设计来源于最大编码率降低（MCR²）的白盒推导：TSSA 不再显式计算 token 之间的相似度矩阵，而是基于 token 特征的二阶统计量（second-order statistics）进行建模；具体而言，TSSA 将 token 软分配到多个子空间（对应注意力头），在每个子空间内估计特征的统计强度，并构造数据依赖的低秩投影算子，对无效或弱相关方向进行抑制；整个过程通过对变分形式的 MCR² 压缩项进行梯度展开得到，使每一层注意力都对应一次“统计压缩”更新，从而实现线性时间与线性显存的注意力计算。



### 3、独特优势

1、**彻底去除 token 两两相似度计算**：
 不同于 Performer、Linformer 等近似方法，TOST 在机制层面不依赖任何形式的 pairwise attention。

2、**线性时间与线性显存复杂度**：
 注意力计算复杂度从 O(n²) 降为 O(n)，在长序列与高分辨率视觉任务中优势显著。

3、**白盒可解释的注意力机制**：
 TSSA 源自优化目标的显式展开，每一层注意力对应一次统计压缩过程，具备清晰的数学与几何解释。

4、**可无缝替换标准自注意力**：
 在不改动 Transformer 主体结构的前提下，直接用 TSSA 替换 Self-Attention，即可获得更高的效率与相近甚至更优的性能。
...
class AttentionTSSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.heads = num_heads                          # 注意力头数

        self.attend = nn.Softmax(dim=1)                 # 在 token 维度上进行 Softmax
        self.attn_drop = nn.Dropout(attn_drop)          # 注意力 Dropout

        self.qkv = nn.Linear(dim, dim, bias=qkv_bias)   # 线性映射生成特征（共享 QKV 表示）

        self.temp = nn.Parameter(torch.ones(num_heads, 1))
        # 每个 head 的可学习温度参数，用于调节注意力分布的平滑程度
    ...
    def forward(self, x):
        # x: [B, N, C]，B 为 batch size，N 为序列长度，C 为特征维度
        ...
        b, h, N, d = w.shape                            # 获取各维度大小

        w_normed = torch.nn.functional.normalize(w, dim=-2)
        # 在 token 维度进行归一化，增强数值稳定性

        w_sq = w_normed ** 2                            # 特征平方，用于能量建模

        # -------- Pi：论文公式 (Eq.10) --------
        Pi = self.attend(
            torch.sum(w_sq, dim=-1) * self.temp
        )
        # 计算每个 token 的重要性权重 Pi，[B, H, N]
        ...
        attn = 1. / (1 + dots)                          # 构造反比例注意力权重
        attn = self.attn_drop(attn)                     # Dropout 正则化

        out = - torch.mul(
            w.mul(Pi.unsqueeze(-1)),
            attn
        )
        # 基于 Pi 和 attn 的联合调制，突出重要 token，抑制冗余响应

        out = rearrange(out, 'b h n d -> b n (h d)')
        # 合并多头输出为 [B, N, C]

        return self.to_out(out)                          # 输出映射
...
论文《Low-Resolution Self-Attention for Semantic Segmentation》

### 1、作用

LRSA 旨在解决语义分割中自注意力计算在高分辨率特征下 FLOPs 与显存开销爆炸的问题，通过在固定低分辨率空间中建模全局上下文，使模型在保持全局感知能力的同时，大幅降低计算复杂度，适配高分辨率语义分割任务。



### 2、机制

LRSA 的核心思想是将自注意力从“高分辨率空间”迁移到“固定低分辨率空间”：首先，将输入特征通过池化下采样到一个固定大小（如 16×16），并在该低分辨率空间中同时生成 Query、Key 和 Value；随后，在低分辨率特征上执行标准自注意力计算，获得全局上下文建模结果；最后，通过双线性插值将注意力结果映射回原始高分辨率空间，并结合 3×3 深度可分离卷积补偿局部细节，从而实现“低分辨率建模全局 + 高分辨率保留细节”的协同设计。
...
1、**注意力复杂度与输入分辨率解耦**：
 自注意力计算在固定大小空间中进行，复杂度由 O(N²) 降为近似 O(N)，适合超高分辨率语义分割。

2、**全局上下文建模能力不丢失**：
 虽然在低分辨率空间计算注意力，但仍保持全局感受野，避免窗口化或局部注意力的信息割裂问题。

3、**局部细节与全局语义兼顾**：
 通过引入高分辨率空间的深度卷积补偿细节，缓解纯低分辨率建模带来的边界与小目标信息损失。

4、**可直接替换标准自注意力**：
 LRSA 可作为通用自注意力替代模块嵌入现有 ViT 或分割 Transformer 中，具备良好的通用性与工程可行性。
...
class LRSA(nn.Module):
    def __init__(
        ...
        # Pyramid Pooling 后 Key/Value 的 token 总数
        ...
        self.pooled_sizes = pooled_sizes                # Pyramid Pooling 尺度列表
        self.pools = nn.ModuleList()                    # 占位（实际由 d_convs 传入）
        self.eps = 0.001                                # 数值稳定项

        self.norm = nn.LayerNorm(dim)                   # LayerNorm（用于 pooled tokens）

        self.q_pooled_size = q_pooled_size              # Query 是否做 pooling

        # ================== Query 卷积分支（可选，原代码中基本不用） ==================
        # 注：作者源码中标注为 Useless code
    ...
    def forward(self, x, H, W, d_convs=None):
        ...
        if self.q_pooled_size > 1:
            ...
            # 对 Query 做固定尺寸的自适应平均池化
            q = F.adaptive_avg_pool2d(
        ...
        # ================== Pyramid Pooling for Key / Value ==================
        pools = []
        x_ = x.permute(0, 2, 1).reshape(B, C, H, W)     # 恢复空间特征图

        for (pooled_size, l) in zip(self.pooled_sizes, d_convs):
            # 根据比例计算每一层 Pyramid Pooling 的尺寸
            pooled_size = (
            ...
            pool = F.adaptive_avg_pool2d(x_, pooled_size)  # 自适应平均池化
            pool = pool + l(pool)                          # 深度卷积增强局部上下文
            pools.append(pool.view(B, C, -1))              # 展平为 token

        pools = torch.cat(pools, dim=2)                    # 拼接多尺度 token
        pools = self.norm(pools.permute(0, 2, 1))          # LayerNorm

        # ================== Key / Value 构建 ==================
        kv = self.kv(pools).reshape(
            B, -1, 2, self.num_heads, C // self.num_heads
        ).permute(2, 0, 3, 1, 4)

        k, v = kv[0], kv[1]                                # 拆分 K 和 V

        # ================== Self-Attention ==================
        attn = (q @ k.transpose(-2, -1)) * self.scale     # 注意力分数
        attn = attn.softmax(dim=-1)
        x = (attn @ v)                                    # 注意力加权输出

        x = x.transpose(1, 2).reshape(B, -1, C)           # 合并 heads

        x = self.proj(x)                                  # 输出投影

        # ================== Query Pooling 时的残差对齐 ==================
        if self.q_pooled_size > 1:
...
if __name__ == "__main__":
    ...
    # 与 pooled_sizes 对应的深度卷积（Pyramid Pooling 的局部增强）
...
论文《HS-FPN: High Frequency and Spatial Perception FPN for Tiny Object Detection》

### 1、作用

HS-FPN 旨在解决微小目标检测中目标特征弱、易被背景淹没、空间对齐不足的问题，通过显式增强高频细节并建模跨层空间依赖，使特征金字塔能够更有效地突出微小目标并提升定位与分类精度。



### 2、机制

HS-FPN 在传统 FPN 结构基础上引入两个关键模块：HFP（High Frequency Perception）模块 通过高通滤波提取特征图中的高频响应，并分别在通道与空间维度生成权重掩码，用于强化与微小目标相关的高频特征、抑制低频背景干扰；SDP（Spatial Dependency Perception）模块 在相邻金字塔层之间引入像素级交叉注意力，显式建模上下层特征之间的空间依赖关系，缓解上采样带来的特征错位问题，从而增强微小目标的空间一致性表达。
...
1、**频域驱动的微小目标增强**：
 通过高频感知机制直接放大微小目标对应的边缘与细节特征，弥补其在深层网络中被逐步削弱的问题。

2、**像素级空间依赖建模**：
 SDP 从像素层面建模上下层特征关系，相比简单逐像素相加的 FPN 融合方式，具备更强的空间感知能力。

3、**对原有检测框架侵入性低**：
 整体结构与 FPN 保持高度一致，可直接替换 FPN 嵌入 Faster R-CNN、Cascade R-CNN 等主流检测框架。

4、**专注微小目标的针对性设计**：
 不同于通用多尺度特征融合方法，HS-FPN 从频域与空间依赖两个角度同时对微小目标进行定向强化，更适合遥感、航拍等场景。
...
import torch.nn as nn
import torch.nn.functional as F
import torch_dct as DCT                      # DCT / IDCT 变换库


# ================== DCT Spatial Interaction ==================
class DctSpatialInteraction(nn.Module):
    def __init__(self,
                 ...
                 isdct=True):
        super().__init__()
        self.ratio = ratio                    # 高频保留比例（H, W 方向）
        self.isdct = isdct                    # 是否启用 DCT 模式
        # isdct=True：使用频域空间交互（P1 / P2）
        # isdct=False：退化为普通空间注意力（P3 / P4）
    ...
    def forward(self, x):
        ...
        # -------- DCT 空间交互 --------
        idct = DCT.dct_2d(x, norm='ortho')    # 2D DCT 变换（频域表示）

        weight = self._compute_weight(h0, w0, self.ratio).to(x.device)
        weight = weight.view(1, h0, w0).expand_as(idct)
        # 高频掩码（低频区域置 0）

        dct = idct * weight                   # 过滤低频，仅保留高频成分
        dct_ = DCT.idct_2d(dct, norm='ortho') # IDCT 回到空间域，生成空间掩码

        return x * dct_                       # 高频引导的空间增强

    def _compute_weight(self, h, w, ratio):
        h0 = int(h * ratio[0])                # 低频高度截断
        w0 = int(w * ratio[1])                # 低频宽度截断
        weight = torch.ones((h, w), requires_grad=False)
        weight[:h0, :w0] = 0                  # 低频区域抑制
        return weight


# ================== DCT Channel Interaction ==================
class DctChannelInteraction(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.in_channels = in_channels
        self.h = patch[0]                     # 通道交互池化高度
        self.w = patch[1]                     # 通道交互池化宽度
        self.ratio = ratio                    # 高频比例
        self.isdct = isdct
    ...
    def forward(self, x):
        ...
        if not self.isdct:
            ...
            channel = (
                self.channel1x1(self.relu(amaxp)) +
                self.channel1x1(self.relu(aavgp))
            )
            return x * torch.sigmoid(self.channel2x1(channel))

        # -------- DCT 通道交互 --------
        idct = DCT.dct_2d(x, norm='ortho')    # DCT 变换

        weight = self._compute_weight(h, w, self.ratio).to(x.device)
        weight = weight.view(1, h, w).expand_as(idct)
        dct = idct * weight                   # 高频保留
        dct_ = DCT.idct_2d(dct, norm='ortho') # 回到空间域
        ...
        channel = (
            self.channel1x1(amaxp) +
            self.channel1x1(aavgp)
        )
...
class HFP(nn.Module):
    ...
    def forward(self, x):
        spatial = self.spatial(x)             # 空间路径输出
        channel = self.channel(x)             # 通道路径输出
        return self.out(spatial + channel)    # 双分支融合
...
论文《InceptionNeXt: When Inception Meets ConvNeXt》

### 1、作用

InceptionNeXt 旨在解决基于大核深度可分离卷积的 CNN（如 ConvNeXt）在实际硬件上吞吐率低、内存访问成本高的问题，在保持大感受野和模型精度的同时，大幅提升训练与推理速度，恢复 CNN 在效率上的核心优势。



### 2、机制

InceptionNeXt 的核心创新在于 Inception Depthwise Convolution：针对 ConvNeXt 中计算代价高但 FLOPs 低效的大核 Depthwise Conv，InceptionNeXt 沿通道维度将输入特征拆分为四个并行分支：一部分通道采用 3×3 小核深度卷积 建模局部信息；两部分通道分别采用 1×k 与 k×1 的带状卷积，在保持大感受野的同时显著降低访存与计算开销；剩余通道直接走 Identity 映射，避免对所有通道进行昂贵的空间混合；最终将各分支特征拼接并送入 ConvNeXt 风格的 Norm + MLP 结构，从而在不引入注意力机制的情况下实现高效全局建模。



### 3、独特优势

1、**显著提升实际吞吐率而非仅降低 FLOPs**：
 通过避免单一大核 Depthwise Conv，有效降低内存访问瓶颈，在 GPU 上获得显著速度提升。

2、**保持大感受野的高效空间建模**：
 利用带状卷积（1×k、k×1）组合替代 k×k 卷积，在几乎不损失建模能力的前提下提升效率。

3、**部分通道参与空间混合的设计更符合效率规律**：
 并非所有通道都需要进行空间交互，Identity 分支减少冗余计算，提升整体性价比。

4、**ConvNeXt 风格、工程友好**：
 结构保持纯 CNN 形式，可直接替换 ConvNeXt 中的 Depthwise Conv，易于在分类、分割等任务中落地。
...
class InceptionDWConv2d(nn.Module):
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125):
        ...
        # ================== 分支通道数计算 ==================
        # gc：每个卷积分支分配的通道数
        # branch_ratio 决定了参与卷积建模的通道比例，其余通道走 identity
        gc = int(in_channels * branch_ratio)



        # ================== 方形深度可分离卷积分支 ==================
        # 使用 square_kernel_size x square_kernel_size 的卷积核
        # groups=gc 表示 depthwise convolution（逐通道卷积）
        ...
        # ================== 水平方向条形深度卷积 ==================
        # 使用 1 x band_kernel_size 的卷积核，增强横向建模能力
        ...
        # ================== 垂直方向条形深度卷积 ==================
        # 使用 band_kernel_size x 1 的卷积核，增强纵向建模能力
    ...
    def forward(self, x):
        ...
        return torch.cat(
            (
                x_id,                     # identity 分支（保留原始特征）
                self.dwconv_hw(x_hw),     # 方形卷积分支
                self.dwconv_w(x_w),       # 水平条形卷积分支
                self.dwconv_h(x_h),       # 垂直条形卷积分支
            ),
            dim=1,
        )
...
论文《CSWin-UNet: Transformer UNet with Cross-Shaped Windows for Medical Image Segmentation》

### 1、作用

CSWin-UNet 旨在解决医学图像分割中全局依赖建模不足与 Transformer 计算开销过高之间的矛盾问题，通过将高效的 CSWin 自注意力机制引入 U-Net 框架，在保持低复杂度的同时增强长程上下文建模能力，从而提升复杂器官与精细边界的分割精度。



### 2、机制

CSWin-UNet 采用纯 Transformer 的 U 型编码器—解码器结构：在编码与解码阶段均使用 CSWin Transformer Block，其核心为 Cross-Shaped Window Self-Attention，将多头注意力拆分为水平条带注意力与垂直条带注意力并行计算，在扩大感受野的同时避免全局注意力的二次复杂度；不同尺度阶段自适应调整条带宽度，使低分辨率阶段获得更大上下文感知能力；在解码端，引入 CARAFE（Content-Aware ReAssembly of Features）上采样算子，通过内容感知的动态核预测实现精细特征重组，配合 U-Net 式跳跃连接，有效恢复空间细节与边界信息。
...
1、**高效的大感受野自注意力机制**：
 CSWin 自注意力通过横纵条带并行建模，实现比窗口注意力更充分的跨区域交互，同时显著降低计算成本。

2、**纯 Transformer 的 U-Net 结构设计**：
 避免 CNN–Transformer 混合带来的结构复杂性，在保持层级特征融合能力的同时提升全局建模一致性。

3、**边界友好的内容感知上采样**：
 CARAFE 相比双线性插值与反卷积，能够更好地保留器官边缘与细粒度结构，适合医学分割任务。

4、**轻量且泛化性强**：
 在多种医学影像模态（CT、MRI、皮肤病灶）上表现稳定，兼顾模型规模、计算效率与分割精度。
...
def img2windows(img, H_sp, W_sp):
    """
    将特征图切分为局部窗口（window partition）

    img: [B, C, H, W]
    return: [B', H_sp * W_sp, C]
    """
    B, C, H, W = img.shape

    # 将特征图 reshape 成窗口结构
    img_reshape = img.view(
        B,
        C,
        H // H_sp, H_sp,
        W // W_sp, W_sp
    ...
    # 调整维度顺序，使窗口成为 batch 维
    img_perm = (
        img_reshape
        .permute(0, 2, 4, 3, 5, 1)   # [B, H/H_sp, W/W_sp, H_sp, W_sp, C]
        .contiguous()
...
def windows2img(img_splits_hw, H_sp, W_sp, H, W):
    ...
    return: [B, H, W, C]
    """
    # 根据窗口数量反推出 batch size
    B = int(img_splits_hw.shape[0] / (H * W / H_sp / W_sp))

    # 恢复窗口网格结构
    img = img_splits_hw.view(
        B,
        H // H_sp, W // W_sp,
        H_sp, W_sp,
        -1
...
class LePEAttention(nn.Module):
    ...
    def __init__(
        ...
        """
        idx 控制窗口切分方式：
        -1 : 全局窗口（H_sp = W_sp = resolution）
         0 : 横向条带（H_sp = resolution, W_sp = split_size）
         1 : 纵向条带（H_sp = split_size, W_sp = resolution）
        """
        if idx == -1:
        ...
        # 用 depthwise conv 生成 Local Positional Encoding（LePE）
    ...
    def im2cswin(self, x):
        ...
        # reshape 为 multi-head attention 形式
        x = (
            x
            .reshape(-1, self.H_sp * self.W_sp, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
    ...
    def get_lepe(self, x, func):
        """
        计算 Local Positional Encoding（LePE）
        """
        B, N, C = x.shape
        H = W = int(np.sqrt(N))
        ...
        x = (
            x
            .view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)
            .permute(0, 2, 4, 1, 3, 5)
            .contiguous()
            .reshape(-1, C, H_sp, W_sp)
        )  # [B', C, H_sp, W_sp]

        # 使用 depthwise conv 生成位置编码
        lepe = func(x)

        # reshape 成 attention 形式
        lepe = (
            lepe
            .reshape(-1, self.num_heads, C // self.num_heads, H_sp * W_sp)
            .permute(0, 1, 3, 2)
            .contiguous()
        )
...
if __name__ == "__main__":
    ...
    resolution = 64
    split_size = 7
    idx = -1        # -1: 全局, 0: 横向, 1: 纵向
    num_heads = 8

    seq_length = resolution * resolution
...
论文《A Semi-supervised Nighttime Dehazing Baseline with Spatial-Frequency Aware and Realistic Brightness Constraint》

### 1、作用

SFSNiD 旨在解决夜间图像去雾中同时存在的雾霾（haze）、光晕（glow）与噪声（noise）相互耦合、频率特性不一致以及亮度失真的问题，通过空间—频率联合建模与半监督亮度约束，实现对真实夜间场景更稳定、自然的去雾增强效果。



### 2、机制

SFSNiD 由空间与频率信息交互模块（SFII）与 半监督亮度约束训练策略协同构成：SFII 模块在特征层面同时建模空间域与频率域信息，先通过傅里叶变换将特征映射到频域，对幅度谱与相位谱进行动态聚合与滤波（FSDA），以区分雾霾、光晕和噪声等具有不同频率特性的退化成分；随后在空间域引入局部注意力机制（Local Perception），并通过双域非线性映射（BNM）融合空间与频率特征，实现对局部退化与细节结构的联合建模；在训练阶段，采用伪标签重训练策略缓解合成数据与真实夜间数据之间的域差异，并引入基于局部窗口的亮度约束损失，利用真实夜间图像的亮度分布作为弱监督信号，抑制过亮或不真实的增强结果。
...
1、**显式建模空间—频率不一致退化**：
 针对夜间雾霾、光晕与噪声在频域上的差异性进行专门设计，优于仅在空间域处理的去雾方法。

2、**双域交互而非简单叠加**：
 SFII 并非独立使用频域或空间域特征，而是通过交互式融合提升对复杂退化的判别能力。

3、**面向真实夜间场景的半监督设计**：
 利用真实夜间图像亮度分布进行约束，有效缓解仅依赖合成数据训练带来的亮度失真问题。

4、**对现有去雾网络侵入性低**：
 SFII 可作为通用特征增强模块嵌入多尺度编码—解码框架中，具有良好的可复用性。
...
# =========================
# SE（Squeeze-and-Excitation）通道注意力层
# =========================
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        # 两层全连接网络，用于建模通道间依赖关系
        self.fc = nn.Sequential(
            nn.Linear(channel, channel // reduction, bias=False),  # 通道降维
            nn.ReLU(inplace=True),                                  # 非线性激活
            nn.Linear(channel // reduction, channel, bias=False),  # 通道升维
            nn.Sigmoid()                                            # 输出通道权重（0~1）
        )

    def forward(self, x):
        b, c, _, _ = x.size()                 # 获取 batch size 和通道数
        y = self.avg_pool(x).view(b, c)       # 全局平均池化并展平
        y = self.fc(y).view(b, c, 1, 1)       # 计算通道权重并恢复维度
        return x * y.expand_as(x)              # 按通道权重重新标定输入特征
...
# =========================
"""
该模块在频域中分别对幅度谱（Magnitude）和相位谱（Phase）进行
通道感知的动态建模与增强，用于区分和处理不同频率成分，例如：
- 低频：雾霾、光照不均
- 中频：结构信息
- 高频：噪声、纹理、光晕
"""
class Frequency_Spectrum_Dynamic_Aggregation(nn.Module):
    ...
    def __init__(self, nc):
        ...
        self.processmag = nn.Sequential(
            nn.Conv2d(nc, nc, kernel_size=1, stride=1, padding=0),  # 点卷积建模通道关系
            nn.LeakyReLU(0.1, inplace=True),                         # 非线性激活
            SELayer(channel=nc),                                     # SE 通道注意力
            nn.Conv2d(nc, nc, kernel_size=1, stride=1, padding=0)   # 输出映射
        )
    ...
    def forward(self, x):
        ...
        """
        Step 2：幅度谱（Magnitude）动态建模
        """
        ori_mag = torch.abs(x_freq)             # 原始幅度谱
        mag = self.processmag(ori_mag)          # 通道感知的动态增强
        mag = ori_mag + mag                     # 残差式增强幅度信息

        """
        Step 3：相位谱（Phase）动态建模
        """
        ori_pha = torch.angle(x_freq)           # 原始相位谱
        pha = self.processpha(ori_pha)          # 通道感知的动态增强
        pha = ori_pha + pha                     # 残差式增强相位信息

        """
        Step 4：由增强后的幅度与相位重建复数频谱
        """
        real = mag * torch.cos(pha)              # 重建实部
        imag = mag * torch.sin(pha)              # 重建虚部
        x_out = torch.complex(real, imag)        # 合成复数频谱

        """
        Step 5：逆 FFT，将频域特征映射回空间域
        """
...
if __name__ == '__main__':
    ...
    block = Frequency_Spectrum_Dynamic_Aggregation(32)   # 实例化模块
    input = torch.rand(4, 32, 64, 64)                    # 随机输入特征
    output = block(input)                                # 前向传播

    print(input.size())   # 输入尺寸
...
论文《Unsupervised Spectral Demosaicing with Lightweight Spectral Attention Networks》

### 1、作用

USD-LSA 旨在解决真实快照式多光谱/高光谱图像无法获得成对真值、监督方法在真实场景中失效的问题，通过构建完全无监督的光谱去马赛克框架，在不依赖合成数据的情况下，实现高质量的光谱—空间重建，并兼顾设备端部署的计算与存储约束。



### 2、机制

USD-LSA 由无监督光谱去马赛克（USD）训练框架与轻量级光谱注意力模块（LSA）协同构成：在训练策略上，USD 采用等变成像思想，将网络输出的光谱立方体进行空间变换（平移、旋转、翻转、缩放）并重新采样为伪马赛克图像，再次输入同一网络，通过 马赛克一致性损失 + 立方体一致性损失实现无监督闭环训练；在网络结构上，引入插值分支作为低频基线，引导网络仅学习高频残差信息，避免模型塌陷为简单复制输入；在特征建模上，LSA 将传统三维光谱注意力张量拆解为空间维度的光谱注意力矩阵与通道维度的光谱注意力向量，分别建模空间相关性与光谱相关性，在大幅减少参数量的同时保持有效的光谱依赖建模能力。
...
1、**真正面向真实数据的无监督训练**：
 无需合成光谱立方体作为监督信号，显著缓解合成—真实数据之间的光谱分布偏差问题。

2、**极致轻量的光谱注意力设计**：
 LSA 相比传统光谱注意力模块减少约 99% 以上参数，更适合资源受限的快照光谱成像设备。

3、**结构性防塌陷机制清晰有效**：
 插值分支 + 残差学习从结构层面约束网络行为，提升无监督训练稳定性。

4、**工程可落地性强**：
 在保持高光谱重建质量的同时，模型参数量与计算复杂度低，适合集成到实际成像硬件中。
...
class LSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # -------------------------------------------------
        # Step 1：将通道展平到 batch 维度
        # [N, C, H, W] → [N*C, 1, H, W]
        # -------------------------------------------------
        x = x.view(N * C, 1, H, W)

        # -------------------------------------------------
        # Step 2：空间 → 通道 shuffle 下采样
        # [N*C, 1, H, W] → [N*C, msfa_size^2, H/msfa, W/msfa]
        # -------------------------------------------------
...
if __name__ == "__main__":
    ...
    height = 256
    width = 256
    msfa_size = 4  # PixelShuffle / ShuffleDown 的缩放因子
